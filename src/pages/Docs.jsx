  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed lg:sticky top-20 left-0 z-40 w-80 h-[calc(100vh-5rem)] bg-gray-900/95 backdrop-blur-md border-r-2 border-gray-800 overflow-y-auto shadow-2xl lg:shadow-none"
      >
    ...
    >
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="prose prose-invert prose-lg max-w-none">
          <div className="bg-gray-800/50 backdrop-blur-sm p-5 sm:p-6 rounded-xl border border-gray-700 mb-6 sm:mb-8">
            <p className="text-gray-300 leading-relaxed text-sm sm:text-base text-center lg:text-left">{chapter.introduction}</p>
          </div>

          {chapter.sections && chapter.sections.map((section, index) => (
            <div key={index} className="mb-6 sm:mb-8">
              <h2 className="text-xl sm:text-2xl font-bold text-white mb-3 sm:mb-4 text-center lg:text-left">{section.heading}</h2>
              <div className="bg-gray-800/30 p-5 sm:p-6 rounded-lg border border-gray-700/50">
                <p className="text-gray-300 leading-relaxed mb-4 text-sm sm:text-base">{section.content}</p>
                {section.points && (
          ...
          {chapter.keyPoints && (
            <div className="bg-blue-900/20 border border-blue-700/50 p-5 sm:p-6 rounded-xl mb-6 sm:mb-8">
              <h3 className="text-lg sm:text-xl font-bold text-blue-400 mb-3 sm:mb-4 text-center lg:text-left">Key Points Summary</h3>
          ...
          {chapter.safetyNotes && (
            <div className="bg-red-900/20 border border-red-700/50 p-5 sm:p-6 rounded-xl mb-6 sm:mb-8">
              <h3 className="text-lg sm:text-xl font-bold text-red-400 mb-3 sm:mb-4 text-center lg:text-left">⚠️ Safety Notes</h3>
              <ul className="space-y-2">
          ...
          {chapter.practicalExample && (
            <div className="bg-green-900/20 border border-green-700/50 p-5 sm:p-6 rounded-xl mb-20">
              <h3 className="text-lg sm:text-xl font-bold text-green-400 mb-3 sm:mb-4 text-center lg:text-left">Practical Example</h3>
              <p className="text-gray-300 leading-relaxed text-sm sm:text-base">{chapter.practicalExample}</p>
            </div>
          )}
        </div>
      </div>
...
const AIAssistantModal = ({ isOpen, onClose }) => {
  ...
  const handleSend = async () => {
    ...
    if (!apiKey) {
      const errorMessage = {
        role: 'assistant',
        content: 'AI Assistant is not configured. Please add your OpenAI API key to the .env file.\n\nSteps:\n1. Get your API key from https://platform.openai.com/api-keys\n2. Add it to .env file: VITE_OPENAI_API_KEY=your_key_here\n3. Restart the development server'
      }
    ...
    try {
      const response = await fetch('https://s', {
        method: 'POST',
        headers: {
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-4 right-4 left-4 sm:left-auto sm:top-auto sm:w-[400px] sm:h-[600px] bg-gray-900 rounded-2xl shadow-2xl z-[60] border-2 border-gray-700 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between p-4 sm:p-6 border-b-2 border-gray-800 flex-shrink-0 bg-gradient-to-r from-gray-900 to-gray-800">
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-400">Ask me anything</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            {/* Language Selector */}
            <div className="px-4 sm:px-6 pt-3 sm:pt-4 pb-2 sm:pb-3 border-b border-gray-800 flex-shrink-0 bg-gray-900">
              <label className="text-xs sm:text-sm font-medium text-gray-300 mb-2 block">Response Language:</label>
              <select
                value={language}
                onChange={(e) => setLanguage(e.target.value)}
                className="w-full px-3 sm:px-4 py-2 sm:py-2.5 bg-gray-800 border-2 border-gray-700 rounded-lg text-white text-sm sm:text-base focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30"
              >
            ...
            {/* Messages Area */}
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-3 sm:space-y-4 min-h-0 bg-gray-950/50">
              ...
              {isLoading && (
                <div className="flex justify-start">
                  <div className="bg-gray-800 p-4 rounded-2xl border-2 border-gray-700">
            ...
            {/* Input Area */}
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-t-2 border-blue-500/30 bg-gray-900 flex-shrink-0">
              <label className="text-xs sm:text-sm font-medium text-blue-400 mb-2 block">Type your question:</label>
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                <textarea
                  ...
                  rows="2"
                  className="flex-1 px-3 sm:px-4 py-2 sm:py-3 bg-gray-800 border-2 border-blue-500/50 rounded-lg text-white text-sm sm:text-base placeholder-gray-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30 resize-none"
                  autoFocus
                />
                <button
                  onClick={handleSend}
